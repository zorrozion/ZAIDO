...
console.log('=== TEST SUITE 1: 真实讲章文本解析与正规化 ===');
const norm1 = normalizeText('弟兄姊妹，今天我们来看以弗所书1:11–14！“随己意行做万事……”');
assert(!norm1.includes('，') && !norm1.includes('！') && !norm1.includes('“'), '标点符号已被正确剔除');
assert(norm1.includes('随己意行做万事'), '核心文字得以保留');
...
console.log('\n=== TEST SUITE 2: 真实讲章文本相似度与关键词打分 ===');
const targetSentence = parsedShort.sentences[1];
...
// 带有口语语气词与同义替换
const fuzzySpeech = '我们在这一段经文当中看到那个保罗讲述的焦点就是从天父上帝永恒中的计划转到耶稣基督十字架上的救赎呢';
...
assert(scoreFuzzy.totalScore >= 0.60, '轻微错词与口语语气词仍应保持高置信度');
...
// 第 0 步
let d1 = matcher.processStep(parsedShort.sentences[0].cleanText, parsedShort.sentences);
console.log(`- 输入第0句语音 -> 位置: ${d1.newIndex}, 状态: ${d1.status}`);
assert(d1.newIndex === 0, '首句保持在 0');

// 顺延讲第 1 句
let d2 = matcher.processStep(parsedShort.sentences[1].cleanText, parsedShort.sentences);
console.log(`- 输入第1句语音 -> 位置: ${d2.newIndex}, 状态: ${d2.status}`);
assert(d2.newIndex === 1, '自然流转推进到第 1 句');

// 顺延讲第 2 句
let d3 = matcher.processStep(parsedShort.sentences[2].cleanText, parsedShort.sentences);
...
// 讲者脱稿结束后重新回归讲稿第 3 句
let recoveryDecision = matcher.processStep(parsedShort.sentences[3].cleanText, parsedShort.sentences);
...
const jumpedSentence = parsedShort.sentences[8];
let jump1 = matcher.processStep(jumpedSentence.cleanText, parsedShort.sentences);
...
let jump2 = matcher.processStep(jumpedSentence.cleanText, parsedShort.sentences);
...
let jump3 = matcher.processStep(jumpedSentence.cleanText, parsedShort.sentences);
...
matcher.processStep(parsedShort.sentences[1].cleanText, parsedShort.sentences);
assert(matcher.processStep('', parsedShort.sentences).newIndex === 8, '大幅向后倒退被向前锁定阻挡，保持在第 8 句');

console.log('\n=== TEST SUITE 7: 7,600+ 字完整主日讲道全篇模拟遍历测试 ===');
const longMatcher = new VoiceFollowMatcher(0);
let simulatedCur = 0;
let forwardSteps = 0;

for (let i = 0; i < parsedFull.sentences.length; i++) {
  const sentence = parsedFull.sentences[i];
  const decision = longMatcher.processStep(sentence.cleanText, parsedFull.sentences);
